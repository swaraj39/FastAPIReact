export const analyzeContent = (config) => {
    return {
        selection: {
...
export const generateHeader = (config, _generationDate) => {
    ...
    const processingInfo = processingNotes.length > 0 ? `The content has been processed where ${processingNotes.join(', ')}.` : '';
    return `${description}, combined into a single document by Repomix.\n${processingInfo}`.trim();
};
export const generateSummaryPurpose = (config) => {
    const info = analyzeContent(config);
...
export const generateSummaryFileFormatJson = () => {
    return `
The content is organized as follows:
1. This summary section
2. Repository information
