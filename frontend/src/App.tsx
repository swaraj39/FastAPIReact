...
import { KeyRound, LayoutDashboard, LogOut, Menu, Shield, Store, UserRound, X } from 'lucide-react'
      ...
      <nav className="sticky top-0 z-50 flex flex-wrap items-center justify-between gap-4 border-b border-line bg-surface px-6 py-[0.7rem] shadow-nav max-[820px]:px-5 max-[820px]:py-3">
        ...
        >
          <span className="inline-flex items-center gap-[0.6rem] whitespace-nowrap text-[1.05rem] font-bold tracking-[0.01em] text-ink">
            <span
              className="inline-flex h-[1.7rem] w-[1.7rem] shrink-0 items-center justify-center rounded-[0.35rem] bg-ink font-serif text-[0.95rem] font-bold text-white"
              aria-hidden="true"
            >
              F
            </span>
        ...
        {/* On mobile the links collapse into a vertical menu that is only
            shown while the hamburger menu is open. */}
        <div
          className={`flex flex-wrap items-center gap-1 max-[820px]:order-3 max-[820px]:w-full max-[820px]:flex-col max-[820px]:items-stretch max-[820px]:gap-[0.15rem] max-[820px]:pt-2 max-[820px]:pb-1 ${
            menuOpen ? 'max-[820px]:flex' : 'max-[820px]:hidden'
          }`}
        >
          {user ? (
            <>
        ...
        {/* Signed-in user: avatar, name and sign-out (hidden on mobile
            until the menu opens). */}
        {user && (
          <div className="flex items-center gap-2 border-l border-line pl-4 max-[820px]:ml-auto max-[820px]:border-l-0 max-[820px]:pl-0">
